...
class GenericDataset(data.Dataset):
    ...
    for ann in anns:
      ...
      if (h > 0 and w > 0):
        ...
        if np.random.random() < self.opt.fp_disturb and reutrn_hm:
          ...
          ct2[0] = ct2[0] + np.random.randn() * self.opt.fp_disturb * 10 * w
          ct2[1] = ct2[1] + np.random.randn() * self.opt.fp_disturb * 10 * h 
  ...
  def _add_instance(
    ...
    if h <= 0 or w <= 0:
      return
    radius = gaussian_radius((self.opt.object_r, self.opt.object_r))
    radius = max(0, int(radius)) 
    ct = np.array(
      [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)
    ct_int = ct.astype(np.int32)
