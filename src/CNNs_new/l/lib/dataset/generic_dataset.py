...
from src.CNNs_new.lib.utils.image import flip, color_aug
from src.CNNs_new.lib.utils.image import get_affine_transform, affine_transform
from src.CNNs_new.lib.utils.image import gaussian_radius, draw_umich_gaussian
import copy
...
class GenericDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    #boxes = [np.array(obj['bbox']) for obj in anns]
    # ret['bboxes'] = boxes
    # ret['hm'] = np.expand_dims(pre_img[0, ...], axis=0)
    return ret
  ...
  def _load_data(self, index):
    ...
    # debug Camilo
    img = img[:self.opt.input_h, :self.opt.input_w, :]
  ...
  def _get_pre_dets(self, anns, trans_input, trans_output):
    ...
    for ann in anns:
      ...
      max_rad = 2
      if (h > 0 and w > 0):
        radius = 12 # gaussian_radius((math.ceil(h), math.ceil(w)))
  ...
  def _get_input(self, img, trans_input):
    ...
    if self.split == 'train' and not self.opt.no_color_aug and False:
  ...
  def _add_instance(
    ...
    if h <= 0 or w <= 0:
      return
    radius = 2 # gaussian_radius((math.ceil(h), math.ceil(w)))
    radius = max(0, int(radius)) 
    ct = np.array(
      [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)
    ct_int = ct.astype(np.int32)
