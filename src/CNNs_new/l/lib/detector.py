...
from src.CNNs_new.lib.model.model import create_model, load_model
from src.CNNs_new.lib.model.decode import generic_decode
from src.CNNs_new.lib.model.utils import flip_tensor, flip_lr_off, flip_lr
from src.CNNs_new.lib.utils.image import get_affine_transform, affine_transform
from src.CNNs_new.lib.utils.image import draw_umich_gaussian, gaussian_radius
from src.CNNs_new.lib.utils.post_process import generic_post_process
from src.CNNs_new.lib.utils.debugger import Debugger
from src.CNNs_new.lib.utils.tracker import Tracker
from src.CNNs_new.lib.dataset.dataset_factory import get_dataset
...
class Detector2(object):
  ...
  def run(self, images, pre_hm=None, pre_ind=None, meta={}):
    ...
    if self.opt.pre_hm:
      ...
      print(pre_hm.shape)
      print(pre_hms2.shape)
      pass
      # print(pre_hms.shape)
      #cv2.imshow("temp", pre_hms[0, 0, ...].cpu().numpy())
      # cv2.waitKey(0)
      # exit()
      # pre_hms = pre_hms
    ...
    # print(images.shape) # torch.Size([576, 544, 3])
    # exit()
...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, pre_hm=None, meta={}):
    ...
    ret = {'results': results, 'tot': tot_time, 'load': load_time,
            ...
            'display': display_time, 'output': output}
  ...
  def _get_additional_inputs(self, dets, meta, with_hm=True):
    ...
    for det in dets:
      ...
      h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
      if (h > 0 and w > 0):
        radius = gaussian_radius((self.opt.object_r_pre, self.opt.object_r_pre))
        radius = max(0, int(radius))
        ct = np.array(
          [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)
        ct_int = ct.astype(np.int32)
