...
import cv2
from src.CNNs_new.lib.detector import Detector
import numpy as np
import csv
from src.CNNs_new.lib.opts import opts as opts_f
...
def create_detector(parameters, rows, cols):
    ...
    opt = opts_f().update_dataset_info_and_set_heads(opt, Dataset)
    opt.test = True
    opt.task = 'tracking'

    opt.arch = 'generic'
    opt.backbone = 'dla34' # 'mobilenet'
    opt.load_model = os.path.join("C:/Users/caguilar/Downloads/CenterTrack-master(2)/CenterTrack_og/src/trained_w_40_41_42_V2_dla34", 'model_last.pth')
