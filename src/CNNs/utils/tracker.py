...
class Tracker(object):
  ...
  def step(self, results, public_det=None):
    ...
    invalid = ((dist > 10 * track_size.reshape(1, M)) + \
      (dist > 10 * item_size.reshape(N, 1)) + \
    ...
    if self.opt.hungarian:
      ...
      dets_m = np.expand_dims(matched_indices[0], 1)
      tracks_m = np.expand_dims(matched_indices[1], 1)
      matched_indices = np.concatenate((dets_m, tracks_m), axis=1)
